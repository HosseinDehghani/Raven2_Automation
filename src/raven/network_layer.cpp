...
#include <sstream>
#include <raven_2/raven_master.h>
void fakeMasterCallback(raven_2::raven_master msg);
static struct u_struct fakeMaster;
...
ros::Subscriber sub = n.subscribe("fake_master", 1000, fakeMasterCallback);
...
//ROS_ERROR("\n\n");
//ROS_ERROR("===============================================");
//ROS_ERROR("_network ros working_");
        rmask = mask;
...
nfound=1;
	...
	u.sequence=fakeMaster.sequence;
	u.pactyp=fakeMaster.pactyp;
	u.version=fakeMaster.version;

	u.delx[0]=fakeMaster.delx[0];
	u.dely[0]=fakeMaster.dely[0];
	u.delz[0]=fakeMaster.delz[0];
	u.Qx[0]=fakeMaster.Qx[0];
	u.Qy[0]=fakeMaster.Qy[0];
	u.Qz[0]=fakeMaster.Qz[0];
	u.Qw[0]=fakeMaster.Qw[0];
	u.buttonstate[0]=fakeMaster.buttonstate[0];
	u.grasp[0]=fakeMaster.grasp[0];

	u.delx[1]=fakeMaster.delx[1];
	u.dely[1]=fakeMaster.dely[1];
	u.delz[1]=fakeMaster.delz[1];
	u.Qx[1]=fakeMaster.Qx[1];
	u.Qy[1]=fakeMaster.Qy[1];
	u.Qz[1]=fakeMaster.Qz[1];
	u.Qw[1]=fakeMaster.Qw[1];
	u.buttonstate[1]=fakeMaster.buttonstate[1];
	u.grasp[1]=fakeMaster.grasp[1];
	u.surgeon_mode=fakeMaster.surgeon_mode;
	u.checksum=fakeMaster.checksum;


	//ROS_ERROR("_network sent_ surgeon mode : %d", u.surgeon_mode);
	//ROS_ERROR("_network sent_ delx : %d", u.delx[0]);
if(fakeMaster.surgeon_mode==1)
{
...
void fakeMasterCallback(raven_2::raven_master msg)
{
	...
	fakeMaster.sequence=msg.sequence;
	fakeMaster.pactyp=msg.pactyp;
	fakeMaster.version=msg.version;

	fakeMaster.delx[0]=msg.delx[0];
	fakeMaster.dely[0]=msg.dely[0];
	fakeMaster.delz[0]=msg.delz[0];
	fakeMaster.Qx[0]=msg.Qx[0];
	fakeMaster.Qy[0]=msg.Qy[0];
	fakeMaster.Qz[0]=msg.Qz[0];
	fakeMaster.Qw[0]=msg.Qw[0];
	fakeMaster.buttonstate[0]=msg.buttonstate[0];
	fakeMaster.grasp[0]=msg.grasp[0];

	fakeMaster.delx[1]=msg.delx[1];
	fakeMaster.dely[1]=msg.dely[1];
	fakeMaster.delz[1]=msg.delz[1];
	fakeMaster.Qx[1]=msg.Qx[1];
	fakeMaster.Qy[1]=msg.Qy[1];
	fakeMaster.Qz[1]=msg.Qz[1];
	fakeMaster.Qw[1]=msg.Qw[1];
	fakeMaster.buttonstate[1]=msg.buttonstate[1];
	fakeMaster.grasp[1]=msg.grasp[1];
	fakeMaster.surgeon_mode=msg.surgeon_mode;
	fakeMaster.checksum=msg.checksum;
}
