...
int fwd_kin (double in_j[6], l_r in_arm, btTransform &out_xform)
{
	...
	if (in_arm == dh_left)
		get_T6(getFKTransform(0,3), getFKTransform(0,4), getFKTransform(0,5));// added by Hossein
...
// Added by Hossein; to get transformation matrices
#include "raven_2/raven_tf.h"
ros::Publisher pub_tf;
int init_raven_tf_publishing(ros::NodeHandle &n){
	pub_tf = n.advertise<raven_2::raven_tf>("/raven_tf", 5000);

}
void get_T6(btTransform T03, btTransform T04, btTransform T05)
{	
	raven_2::raven_tf msg;
	tf::transformTFToMsg(T03, msg.tf_incr[0]);
	tf::transformTFToMsg(T04, msg.tf_incr[1]);
	tf::transformTFToMsg(T05, msg.tf_incr[2]);
	pub_tf.publish(msg);
