...
//ROS_ERROR("________local received_ delx before: %d", us_t->delz[armidx]);
//ROS_ERROR("________local received_ Qx before: %f", us_t->Qx[armidx]);
//ROS_ERROR("________local size delrot1: %d", sizeof(q_temp));
//ROS_ERROR("________local send delrot1: %f", q_temp[0]);
//ROS_ERROR("________local send delrot2: %f", q_temp[1]);
//ROS_ERROR("________local send delrot3: %f", q_temp[2]);
//ROS_ERROR("________local send delrot4: %f", q_temp[3]);
ROS_ERROR("________local received_ data.x before: %d", data1.xd[i].x);
ROS_ERROR("________local received_ p before: %d", p.x);
        fromITP(&p, q_temp, armserial);
ROS_ERROR("________local received_ p after: %d", p.x);
        data1.xd[i].x += p.x;
        data1.xd[i].y += p.y;
        data1.xd[i].z += p.z;

ROS_ERROR("________local received_ data.x after: %d", data1.xd[i].x);
...
//ROS_ERROR("________local update master_ data.x : %d", data1.xd[i].x);
...
void autoincrCallback(raven_2::raven_automove msg)
{
  ...
  for (int i=0;i<2;i++)
	...
	//data1.rd[i].grasp = 1000;//.1*data1.rd[i].grasp;//+1;//msg.grasp[i];


	if (i==1)
	{
		const int graspmax = 3800; //(M_PI * 1000);
		const int graspmin = 1500; //(-30.0 * 1000.0 DEG2RAD);
		data1.rd[i].grasp += msg.grasp[i];
		ROS_ERROR("%d", data1.rd[i].grasp);
		if (data1.rd[i].grasp>graspmax) data1.rd[i].grasp=graspmax;
		else if(data1.rd[i].grasp<graspmin) data1.rd[i].grasp=graspmin;
	}
    } 
